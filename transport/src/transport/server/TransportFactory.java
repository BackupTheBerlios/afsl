...
public class TransportFactory {
	...
	public Transport getTransport(int id) {
		...
		try {
			...
			PreparedStatement st=conn.prepareStatement("select * from transports where id = ?");
	...
	public Transport[] getAllTransports() {
		...
		try {
			Connection conn=ConnectionFactory.getConnection();
			PreparedStatement st=conn.prepareStatement("select * from transports");
			ResultSet rs=st.executeQuery();
			while (rs.next()) {
				newTransport(rs.getInt(1),rs.getDate(2),rs.getDate(3),rs.getDate(4),
	...
	private void insertCars(Transport newTransport, Connection conn) throws SQLException {
		Car[] cars=newTransport.getCars();
		PreparedStatement st=conn.prepareStatement("insert into car_transport (car_id, transport_id) values ( ? , ? )");
	...
	private void insertArtists(Transport newTransport, Connection conn) throws SQLException {
		Artist[] artists=newTransport.getArtists();
		PreparedStatement st=conn.prepareStatement("insert into artist_transport (artist_id, transport_id) values ( ? , ? )");
	...
	private void insertFunctionaries(Transport newTransport, Connection conn) throws SQLException {
	...
	private void insertTransportDropoffs(Transport newTransport, Connection conn) throws SQLException {
	...
	public void newTransport(Transport newTransport) {
		try {
			...
			st.setDate(2,new java.sql.Date(newTransport.getReturnTime().getTime()));
	...
	private void updateFunctionaries(Transport updatedTransport, Connection conn) {
	}

	private void updateTransportDropoffs(Transport updatedTransport, Connection conn) {
	}
	...
	public void updateTransport(Transport updatedTransport) {
		try {
			...
			st.setDate(1,new java.sql.Date(updatedTransport.getStartTime().getTime()));
			st.setDate(2,new java.sql.Date(updatedTransport.getReturnTime().getTime()));
